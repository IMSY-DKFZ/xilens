 ...
 */
FileImage::FileImage(const char *filePath, unsigned int imageHeight,
  ...
  int32_t chunk_shape[] = {1, static_cast<int>(imageHeight),
                           static_cast<int>(imageWidth)};
  int32_t block_shape[] = {1, static_cast<int>(imageHeight),
                           static_cast<int>(imageWidth)};

  this->ctx = b2nd_create_ctx(&storage, 3, shape, chunk_shape, block_shape,
                              "|u2", DTYPE_NUMPY_FORMAT, nullptr, 0);
  int result;
...
void FileImage::AppendMetadata() {
  ...
  PackAndAppendMetadata(this->src, "color_filter_array",
                        this->m_colorFilterArray);
...
void FileImage::write(XI_IMG image) {
  ...
  this->m_colorFilterArray.emplace_back(
      colorFilterToString(image.color_filter_array));
...
void AppendBLOSCVLMetadata(b2nd_array_t *src, const char *key,
  ...
  if (metadataExists < 0) {
    ...
    if (oldoh.get().type == msgpack::type::ARRAY &&
        oldoh.get().via.array.size > 0 &&
    ...
    result = blosc2_vlmeta_update(src->sc, key,
                                  reinterpret_cast<uint8_t *>(sbuf.data()),
                                  sbuf.size(), NULL);
    if (result < 0) {
 ...
 * @return git tag
 */
const char *libfiveGitVersion(void) { return GIT_TAG; }

/**
 * Queries Git revision number
 * @return git revision
 */
const char *libfiveGitRevision(void) { return GIT_REV; }

/**
 * Queries Git branch name
 * @return git branch name
 */
const char *libfiveGitBranch(void) { return GIT_BRANCH; }
