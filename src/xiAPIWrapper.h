...
/**
 * @class XiAPIWrapper
 * @brief This class provides a wrapper for the XiAPI library functions from
 * XIMEA. It can also be overwritten with other implementations such as the
 * Mocks created for unit testing this application.
 */
class XiAPIWrapper {
 public:
  virtual int xiGetParamString(IN HANDLE hDevice, const char* prm, void* val,
                               DWORD size) {
  ...
  virtual int xiSetParamFloat(IN HANDLE hDevice, const char* prm,
                              const float val) {
  ...
  virtual int xiGetImage(IN HANDLE hDevice, IN DWORD timeout,
                         OUT LPXI_IMG img) {
