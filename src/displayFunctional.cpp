 ...
 */
DisplayerFunctional::~DisplayerFunctional() { DestroyWindows(); }
...
void PrepareFunctionalImage(cv::Mat &functional_image,
                            [[maybe_unused]] DisplayImageType displayImage,
                            bool do_scaling, cv::Range bounds, int colormap) {
  if (do_scaling) {
 ...
 * equalization or not.
 */
void DisplayerFunctional::PrepareRawImage(cv::Mat &raw_image,
                                          bool equalize_hist) {
  ...
  raw_image.forEach<Pixel>(
      [mask, this](Pixel &p, const int position[]) -> void {
 ...
 * or 3.
 */
void DisplayerFunctional::DisplayImage(cv::Mat &image,
 ...
 */
void DisplayerFunctional::GetBand(cv::Mat &image, cv::Mat &band_image,
    ...
    if (selected_display % 100 > 0) {
      ...
      if (m_cameraType == CAMERA_TYPE_SPECTRAL) {
        rawImage = cv::Mat::zeros(currentImage.rows / this->m_mosaicShape[0],
                                  currentImage.cols / this->m_mosaicShape[1],
                                  CV_16UC1);
        this->GetBand(currentImage, rawImage, m_mainWindow->GetBand());
        bgrImage =
            cv::Mat::zeros(currentImage.rows / this->m_mosaicShape[0],
        ...
        if (image.color_filter_array == XI_CFA_BAYER_GBRG) {
          ...
          LOG_SUSICAM(error) << "Could not interpret filter array of type: "
                             << image.color_filter_array;
        }

        bgrImage.convertTo(bgrImage, CV_8UC3, 1.0 / m_scaling_factor);
      } else {
        LOG_SUSICAM(error) << "Could not recognize camera type: "
                           << m_cameraType.toStdString();
        throw std::runtime_error("Could not recognize camera type: " +
                                 m_cameraType.toStdString());
      }
...
void DisplayerFunctional::GetBGRImage(cv::Mat &image, cv::Mat &bgr_image) {
  ...
  for (int i : m_bgr_channels) {
    cv::Mat band_image =
        cv::Mat::zeros(image.rows / this->m_mosaicShape[0],
                       image.cols / this->m_mosaicShape[1], CV_16UC1);
 ...
 */
void DisplayerFunctional::DestroyWindows() { cv::destroyAllWindows(); }
