...
class CameraInterface : public QObject
{
  ...
  public:
    /**
     * @brief The CameraInterface class provides an interface for interacting with
     * a camera.
     *
     * This class represents a camera interface that allows control over various
     * camera functionalities such as capturing images, starting and stopping
     * video recording, and adjusting camera settings.
     *
     * The CameraInterface class does not include any example code or
     * implementation details. It serves as a high-level documentation for the
     * camera interface.
     */
     ...
     *
     * This function is used to stop the ongoing acquisition process. It ensures
     * that all resources are properly released and the acquisition is halted.
     *
     ...
     *
     * This function closes the opened device. It performs necessary cleanup and
     * frees any resources used by the device.
     *
     * @note Before calling this function, the device should be opened using the
     * OpenDevice() function.
     *
     * @see OpenDevice()
     */
     ...
     * It is used to keep track of the number of cameras that can be accessed or
     * used for capturing images or recording videos.
     *
     * @note This variable value may change dynamically based on the currently
     * connected cameras, their availability or any configuration changes.
     */
    ...
    /**
     * Custom camera object used to differentiate the actions that need to be
     * taken into account across different types of cameras: gray scale, RGB and
     * spectral. E.g. number of bands, etc.
     */
    std::unique_ptr<Camera> m_camera;

    /**
     * Camera family used to identify the behaviour of different families: xiQ,
     * xiSpec, xiC, etc. E.g. xiSpec support some API calls that xiC does not.
     */
     ...
     *
     * This variable represents the handle used for managing the camera device. It
     * is typically obtained from the camera API and is responsible for retrieving
     * camera data, controlling camera parameters, and performing camera-related
     * operations.
     *
     * The handle is an opaque pointer that provides an abstraction layer for
     * controlling the camera device. Users are strongly advised against modifying
     * or accessing the handle directly.
     *
     * The handle is used for various camera operations, such as opening and
     * closing the camera, starting and stopping video recording, capturing
     * images, and configuring camera settings. It should be properly initialized
     * and released to prevent resource leaks and ensure proper functioning of the
     * camera module.
     *
     * @see https://www.ximea.com/support/wiki/apis/xiapi_manual for more details
     * on how to use the camera handle and its associated functions.
     *
     * @note This variable should be properly initialized before use and released
     * when no longer required to avoid resource leaks and ensure correct behavior
