...
int Camera::InitializeCamera() { return 0; }
...
int Camera::InitializeCameraCommonParameters() {
  ...
  stat = this->m_apiWrapper->xiSetParamInt(*m_cameraHandle,
                                           XI_PRM_IMAGE_DATA_FORMAT, XI_RAW16);
  HandleResult(stat, "xiSetParam (data format raw16)");

  stat = this->m_apiWrapper->xiSetParamInt(*m_cameraHandle, XI_PRM_RECENT_FRAME,
                                           1);
  HandleResult(stat, "xiSetParam (set to acquire most recent frame)");

  stat = this->m_apiWrapper->xiSetParamInt(
      *m_cameraHandle, XI_PRM_AUTO_BANDWIDTH_CALCULATION, XI_ON);
  HandleResult(stat, "xiSetParam (set auto bandwidth calc to on)");

  stat = this->m_apiWrapper->xiSetParamInt(*m_cameraHandle, XI_PRM_GAIN,
                                           XI_GAIN_SELECTOR_ALL);
  ...
  stat = this->m_apiWrapper->xiSetParamInt(*m_cameraHandle,
                                           XI_PRM_OUTPUT_DATA_PACKING, XI_OFF);
  HandleResult(stat, "disable bit packing");

  stat = this->m_apiWrapper->xiSetParamInt(
      *m_cameraHandle, XI_PRM_ACQ_BUFFER_SIZE, 70 * 1000 * 1000);
  HandleResult(stat,
               "set acquistion buffer to 70MB. This should give us a buffer of "
               "about 1s");

  stat = this->m_apiWrapper->xiSetParamFloat(*m_cameraHandle,
                                             XI_PRM_EXP_PRIORITY, 1.);
...
int SpectralCamera::InitializeCamera() {
  ...
  stat = this->m_apiWrapper->xiSetParamInt(
      *m_cameraHandle, XI_PRM_ACQ_TIMING_MODE, XI_ACQ_TIMING_MODE_FRAME_RATE);
  HandleResult(stat, "set acquisition timing mode to framerate");

  stat = this->m_apiWrapper->xiSetParamInt(
      *m_cameraHandle, XI_PRM_DOWNSAMPLING_TYPE, XI_BINNING);
...
int GrayCamera::InitializeCamera() {
  ...
  stat =
      this->m_apiWrapper->xiSetParamInt(*m_cameraHandle, XI_PRM_ACQ_TIMING_MODE,
                                        XI_ACQ_TIMING_MODE_FRAME_RATE_LIMIT);
  HandleResult(stat, "set acquisition timing mode to framerate");

  // XIMEA xiC camera models only allow skipping mode
  stat = this->m_apiWrapper->xiSetParamInt(
      *m_cameraHandle, XI_PRM_DOWNSAMPLING_TYPE, XI_SKIPPING);
...
void XiSpecFamily::UpdateCameraTemperature() {
  ...
  this->m_apiWrapper->xiGetParamFloat(*m_cameraHandle, XI_PRM_CHIP_TEMP,
                                      &chipTemp);
  this->m_apiWrapper->xiGetParamFloat(*m_cameraHandle, XI_PRM_HOUS_TEMP,
                                      &houseTemp);
  this->m_apiWrapper->xiGetParamFloat(
      *m_cameraHandle, XI_PRM_HOUS_BACK_SIDE_TEMP, &houseBackSideTemp);
  this->m_apiWrapper->xiGetParamFloat(*m_cameraHandle, XI_PRM_SENSOR_BOARD_TEMP,
                                      &sensorBoardTemp);
...
void XiQFamily::UpdateCameraTemperature() {
  ...
  if (*m_cameraHandle != INVALID_HANDLE_VALUE) {
    ...
    this->m_apiWrapper->xiGetParamFloat(
        *m_cameraHandle, XI_PRM_HOUS_BACK_SIDE_TEMP, &houseBackSideTemp);
    this->m_apiWrapper->xiGetParamFloat(
        *m_cameraHandle, XI_PRM_SENSOR_BOARD_TEMP, &sensorBoardTemp);
    this->m_cameraTemperature[CHIP_TEMP] = chipTemp;
    this->m_cameraTemperature[HOUSE_TEMP] = houseTemp;
    this->m_cameraTemperature[HOUSE_BACK_TEMP] = houseBackSideTemp;
    this->m_cameraTemperature[SENSOR_BOARD_TEMP] = sensorBoardTemp;
...
void Camera::SetExposure(int exp) {
  ...
  if (INVALID_HANDLE_VALUE != *m_cameraHandle) {
    ...
    stat = this->m_apiWrapper->xiSetParamInt(*m_cameraHandle, XI_PRM_EXPOSURE,
                                             exp);
 ...
 */
void Camera::SetExposureMs(int exp) { this->SetExposure(exp * 1000); }
...
int Camera::GetExposure() {
  ...
  if (INVALID_HANDLE_VALUE != *m_cameraHandle) {
    // Setting "exposure" parameter (10ms=10000us)
    stat = this->m_apiWrapper->xiGetParamInt(*m_cameraHandle, XI_PRM_EXPOSURE,
                                             &exp);
 ...
 */
int Camera::GetExposureMs() { return (this->GetExposure() + 5) / 1000; }
