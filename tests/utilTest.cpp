...
TEST(UtilTest, HandleResultTest) {
  ...
  cam_status = 1;
  ASSERT_THROW(HandleResult(cam_status, "test camera status"),
               std::runtime_error);
}
...
TEST(RescaleTest, CheckValuesAfterRescaling) {
  cv::Mat mat =
      (cv::Mat_<float>(3, 3) << 0.5, 1.2, 2.4, 3.2, 5.1, 6.3, 10.0, 20.0, 15.0);
  rescale(mat, 100.0);
  double min, max;
...
TEST_F(FileImageWriteTest, CheckContentsAfterWriting) {
  ...
  int64_t array_size =
      nrImages * xiImage.width * xiImage.height * sizeof(uint16_t);
  ...
  std::vector<std::string> keys = {"exposure_us", "acq_nframe",
                                   "color_filter_array", "time_stamp"};
