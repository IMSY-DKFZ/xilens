...
class MockMainWindowTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override { mockMainWindow = std::make_unique<MockMainWindow>(); }
};

TEST_F(MockMainWindowTest, UpdateSaturationDisplaysTest_EmptyImage) {
  cv::Mat image;  // Empty image
  ASSERT_THROW(mockMainWindow->UpdateSaturationPercentageLCDDisplays(image),
...
TEST_F(MockMainWindowTest, UpdateSaturationDisplaysTest_ValidImage) {
  cv::Mat image =
      cv::Mat::ones(10, 10, CV_8UC1) * 128;  // Valid grayscale image
...
TEST_F(MockMainWindowTest, WriteLogHeaderTest) {
  ...
  if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    EXPECT_TRUE(line.contains(" git hash: " +
                              QString::fromLatin1(libfiveGitRevision())));

    line = in.readLine();
    EXPECT_TRUE(timestampRegex.match(line).hasMatch());
    EXPECT_TRUE(line.contains(" git branch: " +
                              QString::fromLatin1(libfiveGitBranch())));

    line = in.readLine();
    EXPECT_TRUE(timestampRegex.match(line).hasMatch());
    EXPECT_TRUE(line.contains(" git tags matching hash: " +
                              QString::fromLatin1(libfiveGitVersion())));
