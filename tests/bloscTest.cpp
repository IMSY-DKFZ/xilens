...
void CreateBLOSCArray(char *urlpath) {
  ...
  image->height = height;
  image->bp =
      reinterpret_cast<void *>(new uint16_t[image->width * image->height]());
  int N_images = 10;
  ...
  b2nd_context_t *ctx =
      b2nd_create_ctx(&storage, 3, shape, chunk_shape, block_shape, "|u2",
                      DTYPE_NUMPY_FORMAT, nullptr, 0);
  ...
  std::cout << "Total time spent with b2nd_append: " << total_time
            << " seconds\n";
  ...
  std::vector<std::string> stringArray = {"one", "two", "three", "four",
                                          "five"};
