...
class MockXiAPIWrapper : public XiAPIWrapper {
 public:
  int xiGetParamString(IN HANDLE hDevice, const char* prm, void* val,
                       DWORD size) override {
    return 0;
  }
  ...
  int xiSetParamInt(IN HANDLE hDevice, const char* prm,
                    const int val) override {
    return 0;
  }

  int xiSetParamFloat(IN HANDLE hDevice, const char* prm,
                      const float val) override {
    return 0;
  }

  int xiOpenDevice(IN DWORD DevId, OUT PHANDLE hDevice) override { return 0; }

  int xiCloseDevice(IN HANDLE hDevice) override { return 0; }

  int xiGetNumberDevices(OUT PDWORD pNumberDevices) override { return 0; }

  int xiStartAcquisition(IN HANDLE hDevice) override { return 0; }

  int xiStopAcquisition(IN HANDLE hDevice) override { return 0; }

  int xiGetImage(IN HANDLE hDevice, IN DWORD timeout,
                 OUT LPXI_IMG img) override {
    return 0;
  }
};
...
class MockMainWindow : public MainWindow {
 public:
  explicit MockMainWindow(QWidget* parent = nullptr)
      : MainWindow(parent, std::make_shared<MockXiAPIWrapper>()) {}

  unsigned GetBand() const override { return 8; }
};
...
class MockDisplayerRaw : public DisplayerRaw {
 public:
  MockDisplayerRaw() : DisplayerRaw() { m_mainWindow = new MockMainWindow(); };
